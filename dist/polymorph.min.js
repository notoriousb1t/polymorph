...
var _=void 0,g=" ",c="fill",S="V",b="H",q="L",M="Z",j="M",T="C",C="S",E="Q",H="T",$="A"
var t=Math,F=t.abs,u=t.min,s=t.max,o=t.floor,h=t.round,I=t.sqrt,n=t.pow,L=t.cos,N=t.asin,O=t.sin,Q=t.tan,U=t.PI,V=2/3,f=n(2,-52)
...
return{x:e.x+e.w*r,y:e.y+e.h*t}}function l(r,t,n,e){return I((r-n)*(r-n)+(t-e)*(t-e))}function v(r,t,n,e){var i=e.length
...
return t}}function D(r,t,n){for(var e=r.length,i=p(e),a=0;a<e;a++)i[a]=r[a]+(t[a]-r[a])*n
return i}function z(r,t){return r===_?t:r}var k=120*U/180,B=2*U
function G(r,t,n,e,i,a,o,f,u,c,s,l,v){if(n<=0||e<=0)return[r,t,f,u,f,u]
var g,h=U/180*(+i||0),p=L(h),d=O(h),y=!!c
if(!y){var m=r,x=f,w=((r=m*p-t*-d)-(f=x*p-u*-d))/2,A=((t=m*-d+t*p)-(u=x*-d+u*p))/2,D=w*w/(n*n)+A*A/(e*e)
1<D&&(n*=D=I(D),e*=D)
var z=(a===o?-1:1)*I(F((n*n*e*e-n*n*A*A-e*e*w*w)/(n*n*A*A+e*e*w*w)))
l=z*n*A/e+(r+f)/2,c=N((t-(v=z*-e*w/n+(t+u)/2))/e),s=N((u-v)/e),r<l&&(c=U-c),f<l&&(s=U-s),c<0&&(c+=B),s<0&&(s+=B),o&&s<c&&(c-=B),!o&&c<s&&(s-=B)}if(F(s-c)>k){var P=s,S=f,b=u
g=G(f=l+n*L(s=c+k*(o&&c<s?1:-1)),u=v+e*O(s),n,e,i,0,o,S,b,s,P,l,v)}else g=[]
var q=4/3*Q((s-c)/4)
if(g.splice(0,0,2*r-(r+q*n*O(c)),2*t-(t-q*e*L(c)),f+q*n*O(s),u-q*e*L(s),f,u),!y)for(var M=0,j=g.length;M<j;M+=2){var T=g[M],C=g[M+1]
g[M]=T*p-C*d,g[M+1]=T*d+C*p}return g}var J={M:2,H:1,V:1,L:2,Z:0,C:6,S:4,Q:4,T:2,A:7}
...
else if(o===T)K(t,l[0],l[1],l[2],l[3],l[4],l[5]),t.cx=l[2],t.cy=l[3]
else if(o===C){var w=t.lc!==C&&t.lc!==T
K(t,h=w?_:2*v-t.cx,p=w?_:2*g-t.cy,l[0],l[1],l[2],l[3]),t.cx=l[0],t.cy=l[1]}else if(o===E){var A=l[0],D=l[1]
d=l[2],y=l[3],K(t,v+(A-v)*V,g+(D-g)*V,d+(A-d)*V,y+(D-y)*V,d,y),t.cx=A,t.cy=D}else if(o===H)d=l[0],y=l[1],t.lc===E||t.lc===H?(h=v+(2*v-t.cx-v)*V,p=g+(2*g-t.cy-g)*V,m=d+(2*v-t.cx-d)*V,x=y+(2*g-t.cy-y)*V):(h=m=v,p=x=g),K(t,h,p,m,x,d,y),t.cx=m,t.cy=x
else if(o===$)for(var z=G(v,g,l[0],l[1],l[2],l[3],l[4],l[5],l[6]),P=0;P<z.length;P+=6)K(t,z[P],z[P+1],z[P+2],z[P+3],z[P+4],z[P+5])
...
var n=r
return"PATH"===n.tagName?{data:X(t=n.getAttribute("d")),stringData:t}:Z("Unsupported element ",n.tagName)}(r),n=t.data,e=t.stringData
