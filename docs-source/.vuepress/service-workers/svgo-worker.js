...
function cloneParsedSvg(svg) {
  ...
  function clone(obj) {
    ...
    } else if (obj.constructor === CSSClassList || obj.constructor === CSSStyleDeclaration || obj.constructor === Object || obj.constructor === Array) {
...
function* multipassCompress(settings) {
  ...
  Object.keys(settings.plugins).forEach(pluginName => {
    const plugin = pluginsData[pluginName];
    if (plugin) {
      plugin.active = settings.plugins[pluginName];
    }
  });
